// src/app/site/services/[id]/page.tsx
"use client";
...
interface ServiceDetailsClientProps {
    subData: any; // replace `any` with your actual type if known
}
// --- COLORS ---
...
type CartRow = { service_id: number; quantity: number; selected_services?: string[] | null };
...
// ------------------- MAIN COMPONENT -------------------
const ServiceDetailsClient: React.FC<ServiceDetailsClientProps> = ({ subData }) => {
    ...
    const fetchReviewsForSubcategory = useCallback(async (subcategoryName: string) => {
        const { data: serviceIdsData } = await supabase.from("services").select("id").eq("subcategory", subcategoryName);
        ...
        reviewsData.forEach((review) => {
            if (!review.service_id || !serviceIds.includes(review.service_id)) return;
            if (!ratingsMap[review.service_id]) ratingsMap[review.service_id] = { sum: 0, count: 0 };
        ...
        const allSubcategoryReviews = reviewsData.filter((r) => serviceIds.includes(r.service_id));
        const totalRating = allSubcategoryReviews.reduce((sum, r) => sum + r.rating, 0);
        setServiceStats({
            averageRating: allSubcategoryReviews.length > 0 ? totalRating / allSubcategoryReviews.length : null,
    ...
    const fetchReviewsForService = useCallback(async (serviceId: number) => {
        ...
        if (error) return setReviewsForService([]);
    ...
    const confirmAddToCart = useCallback(async (item: ServiceItem, selected_services: string[]) => {
        if (!isAuthenticated || !userId) return alert("Login required!");
        ...
        if (error) return console.error("Error adding to cart:", error);
        if (data) {
            setCartItems(prev => {
    ...
    const handleCartClick = (service: ServiceItem, isInCart: boolean) => {
        if (!isAuthenticated) return setLoginPromptOpen(true);
        if (isInCart) return router.push("/site/cart");
